	...
	try:
		...
		try:
			...
			print 'Missing a required configuration option'
                ...
                cursor = db.cursor()
                sql="SELECT CONCAT(TABLE_SCHEMA,'.',TABLE_NAME) AS TABLE_NAME, (DATA_FREE/DATA_LENGTH) AS FRAG_RATIO FROM TABLES WHERE ENGINE = 'MyISAM' AND (DATA_LENGTH > 0 OR DATA_FREE > 0) AND (DATA_FREE/DATA_LENGTH) >=%s" % (1.00*frag_thresh/100)
                cursor.execute(sql)
                res = cursor.fetchall()
		i = 0
                for row in res:
